...
namespace ProjectAltis.Core
{
    ...
    {
        private static string CertificateFingerprint = "102F4358D519F726EF517257F10D2799";
        ...
        public static bool IsSignedByAltis(string filename)
        {
            if (!File.Exists(filename))
                return false;
            try
            {
                ...
                var valid = cert.GetSerialNumberString() == CertificateFingerprint;
                Log.Info($"Verifying file {filename} with certificate serial number {CertificateFingerprint}, result: {valid}");
                return valid;
            }
            catch (Exception e)
            {
